...
def _write_backup(frame, backup_output_folder):
    """Internal helper to write a timestamped backup copy if folder provided"""
    try:
        if backup_output_folder and frame is not None:
            ...
            print(f"Backup wrote to {backup_path}: {ok2}")
...
def main(part, subpart, frame,
    ...
    try:
        ...
        try:
            ...
            if frame is not None:
                ...
                _write_backup(frame, backup_output_folder)  # backup
    ...
    try:
        ...
        if part == "PISTON":
            ...
            save_result = dt.save_final_result_image(processed["image"], dims, flash, conc, orifice_data,
        ...
        # 2) Ring-like: ID/OD + concentricity (no orifice)
            ...
            save_result = dt.save_final_result_image(processed["image"], dims, flash, conc, None,
                                                     output_folder=output_folder,
                                                     backup_output_folder=backup_output_folder)  # new
            result['image_path'] = save_result.get('output_path')
        ...
        # 3) O RING: ID only (OD NA), no concentricity/orifice
            ...
            result['measurements'].pop('orifice', None)
            save_result = dt.save_final_result_image(processed["image"], dims, flash, None,
                                                     output_folder=output_folder,
                                                     backup_output_folder=backup_output_folder)  # new
            result['image_path'] = save_result.get('output_path')

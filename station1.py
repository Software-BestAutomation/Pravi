...
# # ## updated 20\9\2025  11:PM   upto 27_sep_25   11pm
# import defect as dt
# import cv2
# import numpy as np
...
#             image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)
#         else:
#             # If it's 3D with 1 channel, squeeze and convert
#             image = cv2.cvtColor(image.squeeze(), cv2.COLOR_GRAY2BGR)
    
#     # Now perform the desired conversion
#     try:
#         result = cv2.cvtColor(image, conversion_code)
#         return result
#     except cv2.error as e:
...
#         # Check and handle frame channels FIRST
#         print(f"Original frame shape: {frame.shape}")
        
#         # Ensure frame has 3 channels (BGR) for all subsequent processing
#         if len(frame.shape) == 2:
#             # Grayscale image - convert to BGR
#             frame = cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR)
#             print("Converted grayscale frame to BGR")
#         elif len(frame.shape) == 3 and frame.shape[2] == 1:
#             # Single channel in 3D array - squeeze and convert
#             frame = cv2.cvtColor(frame.squeeze(), cv2.COLOR_GRAY2BGR)
#             print("Converted single-channel frame to BGR")
#         elif len(frame.shape) == 3 and frame.shape[2] == 4:
...
#         # Extract values for shortcut return format
#         resultType = "r"
#         result_status = "NOK" if defect_lines else "OK"
#         ID = str(result['measurements']['id']['value'])
#         IDstatus = result['measurements']['id']['status']
#         OD = str(result['measurements']['od']['value'])
#         ODstatus = result['measurements']['od']['status']
#         Concentricity = str(result['measurements']['concentricity']['value']) if 'concentricity' in result['measurements'] else "NA"
#         ConcentricityStatus = result['measurements']['concentricity']['status'] if 'concentricity' in result['measurements'] else "NA"
#         FlashDefect = result['measurements']['flash']['result']
#         DefectPosition = result['measurements']['flash']['position']
#         OrificeDiameter = str(result['measurements']['orifice']['value']) if 'orifice' in result['measurements'] else "NA"
#         OrificeStatus = result['measurements']['orifice']['status'] if 'orifice' in result['measurements'] else "NA"
#         dim_err = ", ".join(defect_lines) if defect_lines else "None"

#         # print statement (optional - you can remove these if not needed)
#         print("r")
#         print("Result:", result_status)
#         if defect_lines:
#             print("Error:", dim_err)
...
#         # Return in shortcut format for error case
#         return ("e", "NOK", "0.0", "NOK", "0.0", "NOK", "NA", "NA", "NOK", "None", "NA", "NA", str(e))

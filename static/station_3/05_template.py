...
# [P0] CONFIGURATION
# - Area gates define which contours are considered the washer OD/ID.
# - IN_DIR is a directory mode; FILES is a direct list mode.
# - SHOW_SIZE controls a uniform 2D size for all displayed and saved visualizations.
...
FILES = [
    ...
    # "D:/Pravi/burr_images/new_glass/station3/dataset/9_oct_25/VCXG.2-32C/image0000391.bmp",
    "D:/Pravi/burr_images/new_glass/station3/dataset/9_oct_25/VCXG.2-32C/image0000392.bmp",
...
# [P1] RING CONTEXT: detect bright circular ring for annotation/tuning (not required for logic)
# Uses HoughCircles on blurred grayscale to get (x, y, r); returns None if not found.
# Reference: Hough Circle Transform tutorial (dp, param1/2, radius limits) [web:27].
...
# [P2] CONTOUR PICKER: choose the best contour in an area window
# Scans all contours, returns index and area of the largest candidate within [min,max].
# Contours are sequences of points; see OpenCV contour docs [web:69].
...
# [P3] MAIN PROCESSOR FOR ONE IMAGE
def process_one(path):
    # [P3.1] LOAD & PREP
    img = cv.imread(str(path))  # BGR image load; None if unreadable [web:59]
    if img is None:
        print(f"Skip unreadable: {path}")
        return
    vis = img.copy()            # visualization canvas to draw on
    gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)  # grayscale for robust ops
    gray = cv.medianBlur(gray, 5)               # denoise salt/pepper before Hough/threshold

    # [P3.2] OPTIONAL RING ANNOTATION
    # Draw detected ring to help visualize setup; independent of contour logic [web:27].
    ...
    # [P4] GLOBAL THRESHOLD (OTSU)
    # Otsu automatically selects a binary threshold, great for bimodal bright objects [web:83].
    _, thr = cv.threshold(gray, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)

    # [P5] MORPHOLOGY CLEANUP
    # Opening removes thin bright noise (e.g., ring lobes), Closing fills small holes in the washer [web:82].
    k_open  = cv.getStructuringElement(cv.MORPH_ELLIPSE, (5,5))
    k_close = cv.getStructuringElement(cv.MORPH_ELLIPSE, (11,11))
    binm = cv.morphologyEx(thr, cv.MORPH_OPEN,  k_open)   # erosion→dilation [web:82]
    binm = cv.morphologyEx(binm, cv.MORPH_CLOSE, k_close) # dilation→erosion [web:82]

    # [P6] FIND CONTOURS + HIERARCHY
    # RETR_TREE gives full nesting relationships so the inner hole (ID) can be found as a child of OD [web:69][web:78].
    ...
    # [P7] SELECT OD BY AREA WINDOW
    od_idx, od_area = find_best_contour_by_area(cnts, OD_AREA_MIN, OD_AREA_MAX)

    # [P8] SELECT ID (prefer hierarchy child of OD; else global fallback)
    id_idx, id_area = None, None
    if od_idx is not None and hier.size:
        # Each hierarchy entry: [Next, Prev, FirstChild, Parent]; Parent==od_idx gives children [web:78].
        for j, c in enumerate(cnts):
            if hier[0][j][3] == od_idx:
                ...
                if ID_AREA_MIN <= a <= ID_AREA_MAX:
                    ...
                    break
    if id_idx is None:
        # Fallback if the hole was not linked as a child (breaks/gaps)
        id_idx, id_area = find_best_contour_by_area(cnts, ID_AREA_MIN, ID_AREA_MAX)

    # [P9] ANNOTATION (DRAW + TEXT)
    # Draw OD/ID contours, centroid for OD using spatial moments, and status text [web:69][web:84].
    status = "PART ABSENT"
    if od_idx is not None:
        cv.drawContours(vis, [cnts[od_idx]], -1, (0,255,0), 2)
        M = cv.moments(cnts[od_idx])           # moments for centroid [web:84]
        if M["m00"] != 0:
    ...
    # [P10] RESIZE VISUALIZATIONS (uniform display/save size)
    # INTER_AREA is preferred for downscaling quality; convert binary to BGR for consistent saving [web:54].
    ...
    # [P11] SAVE RESULTS (resized visualizations)
    # imwrite infers the encoder from the extension; PNG gives lossless, portable outputs [web:59].
    ...
    # [P12] DISPLAY WINDOWS (press any key to continue)
    cv.imshow("result", vis_show)
    # cv.imshow("binary", binm_show)  # uncomment to view binary as well
    cv.waitKey(0); cv.destroyAllWindows()

# [P13] DRIVER: collect inputs either from directory or FILES list and run
if IN_DIR.exists():

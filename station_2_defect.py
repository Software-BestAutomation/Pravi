# code 3 (station_2_defect.py)
# Changes:
# - save_thickness_result_image now accepts backup_output_folder and writes a timestamped backup copy.
# - All measurement/detection logic remains unchanged; only the save routine was extended for backups.
...
def preprocess_image(frame, output_folder=None, min_thresh=50, max_thresh=255):
    ...
    try:
        min_thr = int(float(min_thresh)); min_thr = max(0, min(255, min_thr))
        max_thr = int(float(max_thresh)); max_thr = max(0, min(255, max_thr))
    except Exception:
        min_thr = 0; max_thr = 255
...
def save_thickness_result_image(image, thickness_data, flash_data, output_folder="output_images",
    ...
    """Save result image with thickness annotations, preserving legacy cam2_bmp.bmp and adding an optional timestamped backup."""
    try:
        ...
        if center_seg is not None:
            ...
            cv2.line(result_img, draw_pt1, draw_pt2, (0, 0, 255), 2)
            cv2.circle(result_img, draw_pt1, 3, (0, 255, 255), -1)
            cv2.circle(result_img, draw_pt2, 3, (0, 255, 255), -1)
        ...
        cv2.putText(result_img, thickness_text, (thickness_text_x, thickness_text_y),
        ...
        # Last resort: save original and attempt a backup too
        try:
            os.makedirs(output_folder, exist_ok=True)

...
# # station_3_defect.py      combine ID & OD  [web:2]
# # updated on 22_9_2025 (add combined_output_image for ID+OD in one frame; keep existing behavior)  [web:2]
# # updated on 22_9_2025 (keep ID/OD cropped outputs; add combined cropped overlays for ID and OD)  [web:2]
# import cv2
# import numpy as np
# import math
# import os
...
# def _circularity(contour):
#     # Compute circularity = 4πA/P^2 for a contour; higher values indicate more circular shapes  [web:2]
#     area = cv2.contourArea(contour)
#     perim = cv2.arcLength(contour, True)
#     if perim <= 0:
#         return None, area, perim
...
# def _aspect_ratio(contour):
#     # Compute aspect ratio from bounding rectangle w/h to filter elongated or squashed contours  [web:2]
#     x, y, w, h = cv2.boundingRect(contour)
#     if h == 0:
#         return None, (w, h)
#     return float(w) / float(h), (w, h)
...
# def _passes_shape_filters(contour, min_circularity, max_circularity, min_aspect_ratio, max_aspect_ratio):
#     # Apply optional circularity and aspect-ratio windows to accept/reject a contour candidate  [web:2]
#     if min_circularity is not None or max_circularity is not None:
#         c, _, _ = _circularity(contour)
#         if c is None:
#             return False
...
#                      min_aspect_ratio=None, max_aspect_ratio=None,
#                      output_folder=None):
#     # Convert to grayscale, threshold to binary, and invert so part edges become white for contour detection; then find and sort contours  [web:12]
#     if output_folder:
#         os.makedirs(output_folder, exist_ok=True)
#     gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
#     _, binary = cv2.threshold(gray, 128, 255, cv2.THRESH_BINARY)
...
#     contours, _ = cv2.findContours(binary_inv, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
#     sorted_contours = sorted(contours, key=cv2.contourArea, reverse=True)
#     # Select ID contour by area window and optional shape filters (circularity/aspect ratio)  [web:2]
#     id_contour = None
#     if min_id_area is not None and max_id_area is not None:
#         for contour in sorted_contours:
#             area = cv2.contourArea(contour)
...
#                     id_contour = contour
#                     break
#     # Select OD contour by area window and optional shape filters (circularity/aspect ratio)  [web:2]
#     od_contour = None
#     if min_od_area is not None and max_od_area is not None:
#         for contour in sorted_contours:
#             area = cv2.contourArea(contour)
...
#                     od_contour = contour
#                     break
#     # Build a quick debug image showing the chosen ID/OD contours for verification  [web:2]
#     contour_img = frame.copy()
#     if id_contour is not None:
#         cv2.drawContours(contour_img, [id_contour], -1, (0, 255, 0), 3)
#     if od_contour is not None:
...
# def crop_around_contour(image, contour, crop_size=400):
#     # Crop a square ROI centered at contour centroid using moments; bounds are clamped to image limits  [web:2]
#     h, w = image.shape[:2]
#     m = cv2.moments(contour)
#     if m['m00'] == 0:
#         center = tuple(contour[0][0])
...
#     x_end = min(center[0] + crop_size, w)
#     y_end = min(center[1] + crop_size, h)
#     return image[y_start:y_end, x_start:x_end]  # [web:2]

# def _get_contour_props(contour):
#     # Compute area, centroid, and an area-derived radius (for band construction) for a ring-like contour  [web:2]
#     if contour is None or len(contour) < 5:
#         return None
#     area = cv2.contourArea(contour)
#     m = cv2.moments(contour)
...
#         r = int(np.sqrt(area / np.pi))
#         return {'area': area, 'center': (cx, cy), 'radius': r}
#     return {'area': area, 'center': tuple(contour[0][0]), 'radius': 0}  # [web:2]

# def _filled_mask_from_contour(gray, contour):
#     # Create a binary mask filled inside the given contour to serve as an inner or outer band boundary  [web:2]
#     mask = np.zeros_like(gray, dtype=np.uint8)
#     cv2.drawContours(mask, [contour], -1, 255, cv2.FILLED)
#     return mask  # [web:2]
...
#                   HIGHLIGHT_SIZE, burr_area_min, burr_area_max, burr_perim_min, burr_perim_max,
#                   draw_id=None, draw_od=None, crop_contour=None):
#     # Build a ring zone by subtracting inner from outer; restrict edges to this band to search localized burrs  [web:97]
#     analysis_zone = cv2.subtract(outer_mask, inner_mask)
#     zone_edges = cv2.bitwise_and(edges, edges, mask=analysis_zone)
#     # Find edge fragments in the band and filter by area/perimeter to retain burr-sized contours  [web:2]
#     edge_contours, _ = cv2.findContours(zone_edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
#     burrs = []
#     for e in edge_contours:
#         ea = cv2.contourArea(e)
...
#         if (burr_area_min <= ea <= burr_area_max) and (burr_perim_min <= ep <= burr_perim_max):
#             burrs.append(e)
#     # Draw selected contours and highlight burrs with fixed-size rectangles for visualization  [web:2]
#     out = image.copy()
#     if draw_id is not None:
#         cv2.drawContours(out, [draw_id], -1, (0, 255, 0), 4)
#     if draw_od is not None:
...
#                       (cx2 + HIGHLIGHT_SIZE // 2, cy2 + HIGHLIGHT_SIZE // 2),
#                       (255, 0, 0), 3)
#     # Add status banner (OK/NOK) and optionally crop around the target contour for focused review  [web:2]
#     status_text = "NOK - BURR DETECTED" if burrs else "OK - NO BURR"
#     status_color = (0, 0, 255) if burrs else (0, 255, 0)
#     tsize = cv2.getTextSize(status_text, cv2.FONT_HERSHEY_SIMPLEX, 2, 5)[0]
#     tx = (out.shape[1] - tsize[0]) // 2
...
#     # Ensure all required parameters exist; otherwise return structured NOK with error and empty overlays  [web:2]
#     required = [id_offset, id_highlight, id_burr_area_min, id_burr_area_max, id_burr_perim_min, id_burr_perim_max,
#                 od_offset, od_highlight, od_burr_area_min, od_burr_area_max, od_burr_perim_min, od_burr_perim_max,
#                 min_id_area, max_id_area, min_od_area, max_od_area]
#     if any(v is None for v in required):
...
#     # If caller did not pass selected contours, fall back to selecting by area and shape filters from the sorted list  [web:2]
#     if id_contour is None:
#         for c in sorted_contours:
#             a = cv2.contourArea(c)
#             if min_id_area <= a <= max_id_area and _passes_shape_filters(c, min_circularity, max_circularity, min_aspect_ratio, max_aspect_ratio):
...
#     # Prepare grayscale and Canny edges for burr extraction along ring bands at ID and OD edges  [web:97]
#     gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
#     edges = cv2.Canny(gray, 90, 100)  # [web:97]

#     results = {"id": None, "od": None}
#     id_burrs = []
#     od_burrs = []

#     # ID burr process: outward ring (inner = filled ID, outer = expanded circle by id_offset) to catch flashes/burrs just outside ID  [web:2]
#     if id_contour is not None:
#         id_props = _get_contour_props(id_contour)
#         cx, cy = id_props['center']
#         r_id = id_props['radius']
...
#     # OD burr process: inward ring (outer = filled OD, inner = shrunk circle by od_offset) to catch flashes/burrs just inside OD  [web:2]
#     if od_contour is not None:
#         od_props = _get_contour_props(od_contour)
#         cx, cy = od_props['center']
#         r_od = od_props['radius']
...
#     # Compose a full-frame combined overlay with both contours and burr highlight boxes plus per-target status text  [web:2]
#     combined_full = frame.copy()
#     if id_contour is not None:
#         cv2.drawContours(combined_full, [id_contour], -1, (0, 255, 0), 4)
#     if od_contour is not None:
...
#         cv2.rectangle(combined_full, (cx2 - od_highlight // 2, cy2 - od_highlight // 2),
#                       (cx2 + od_highlight // 2, cy2 + od_highlight // 2), (255, 0, 0), 3)
#     # Add compact textual status for ID and OD on the combined view to aid quick review  [web:2]
#     id_txt = f"ID: {'NOK' if id_burrs else 'OK'} ({len(id_burrs)})"
#     od_txt = f"OD: {'NOK' if od_burrs else 'OK'} ({len(od_burrs)})"
#     id_color = (0, 0, 255) if id_burrs else (0, 255, 0)
#     od_color = (0, 0, 255) if od_burrs else (0, 255, 0)
...
#     # Create combined crops centered on ID and OD to mimic prior per-target cropped outputs but with a combined overlay  [web:2]
#     combined_id_crop = crop_around_contour(combined_full, id_contour, crop_size=300) if id_contour is not None else None
#     combined_od_crop = crop_around_contour(combined_full, od_contour, crop_size=300) if od_contour is not None else None

#     # Populate timing and pack all outputs for caller  [web:2]
#     elapsed_ms = (time.time() - start) * 1000.0
#     for k in ("id", "od"):
#         results[k]["time_ms"] = elapsed_ms
#     results["combined_output_image"] = combined_full
#     results["combined_output_image_id_crop"] = combined_id_crop
#     results["combined_output_image_od_crop"] = combined_od_crop
#     return results  # [web:2]

# def save_burr_result_image(image, burr_output_image, output_folder="output_images", filename="cam3_id.bmp"):
#     # Save an overlay image if provided; otherwise save the original image; ensures folder exists before write  [web:2]
#     try:
#         os.makedirs(output_folder, exist_ok=True)
#         result_img = burr_output_image if burr_output_image is not None else image.copy()
#         output_path = os.path.join(output_folder, filename)
#         ok = cv2.imwrite(output_path, result_img)
#         return {"output_path": output_path if ok else None, "success": bool(ok)}
#     except Exception as e:
#         return {"output_path": None, "success": False, "error": str(e)}  # [web:2]
...
#     x_end = min(center[0] + crop_size, w)
#     y_end = min(center[1] + crop_size, h)
#     return image[y_start:y_end, x_start:x_end]  # [web:1][web:120]
...
#         r = int(np.sqrt(area / np.pi))
#         return {'area': area, 'center': (cx, cy), 'radius': r}
#     return {'area': area, 'center': tuple(contour[0][0]), 'radius': 0}  # [web:1][web:125]

# def _filled_mask_from_contour(gray, contour):
#     mask = np.zeros_like(gray, dtype=np.uint8)
#     cv2.drawContours(mask, [contour], -1, 255, cv2.FILLED)
#     return mask  # [web:122][web:6]
...
#     if crop_contour is not None:
#         out = crop_around_contour(out, crop_contour, crop_size=300)
#     return burrs, out  # [web:121][web:40]
...
#     # Fallback contour selection if not provided
#     if id_contour is None:
#         for c in sorted_contours:
#             a = cv2.contourArea(c)
#             if min_id_area <= a <= max_id_area and _passes_shape_filters(c, min_circularity, max_circularity, min_aspect_ratio, max_aspect_ratio):
...
#     gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
#     edges = cv2.Canny(gray, 90, 100)  # [web:12][web:32]
...
#     # Build a combined full-frame overlay with both ID and OD
#     combined_full = frame.copy()
#     if id_contour is not None:
#         cv2.drawContours(combined_full, [id_contour], -1, (0, 255, 0), 4)
#     if od_contour is not None:
...
#     # Create combined crops that respect prior behavior
#     combined_id_crop = crop_around_contour(combined_full, id_contour, crop_size=300) if id_contour is not None else None
#     combined_od_crop = crop_around_contour(combined_full, od_contour, crop_size=300) if od_contour is not None else None
...
#     results["combined_output_image_id_crop"] = combined_id_crop
#     results["combined_output_image_od_crop"] = combined_od_crop
#     return results  # [web:122][web:121]
...
#         return {"output_path": output_path if ok else None, "success": bool(ok)}
#     except Exception as e:
#         return {"output_path": None, "success": False, "error": str(e)}  # [web:12][web:40]
...
# # updated on 20_9_2025 (adds circularity/aspect ratio filtering)
# import cv2
# import numpy as np
# import math
...
#     gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
#     _, binary = cv2.threshold(gray, 128, 255, cv2.THRESH_BINARY)
#     print("DEBUG: Applied binary thresholding with threshold=128.")
...
#                 if _passes_shape_filters(contour, min_circularity, max_circularity, min_aspect_ratio, max_aspect_ratio):
#                     id_contour = contour
#                     print(f"DEBUG: Found ID contour idx={i} area={area} passed shape filters")
#                     break
#     if id_contour is None:
...
#                 if _passes_shape_filters(contour, min_circularity, max_circularity, min_aspect_ratio, max_aspect_ratio):
#                     od_contour = contour
#                     print(f"DEBUG: Found OD contour idx={i} area={area} passed shape filters")
#                     break
#     if od_contour is None:
...
#     # Fallback selection here too with shape constraints if preprocessing didn't supply
#     if id_contour is None:
#         print(f"DEBUG: ID contour not provided, searching with area+shape filters...")
#         for i, contour in enumerate(sorted_contours):
#             area = cv2.contourArea(contour)
#             if min_id_area <= area <= max_id_area:
#                 if _passes_shape_filters(contour, min_circularity, max_circularity, min_aspect_ratio, max_aspect_ratio):
#                     id_contour = contour
#                     print(f"DEBUG: (fallback) Found ID idx={i} area={area} passed shape filters")
#                     break

#     if od_contour is None:
#         print(f"DEBUG: OD contour not provided, searching with area+shape filters...")
#         for i, contour in enumerate(sorted_contours):
#             area = cv2.contourArea(contour)
#             if min_od_area <= area <= max_od_area:
#                 if _passes_shape_filters(contour, min_circularity, max_circularity, min_aspect_ratio, max_aspect_ratio):
#                     od_contour = contour
#                     print(f"DEBUG: (fallback) Found OD idx={i} area={area} passed shape filters")
#                     break
...
#     _, binary = cv2.threshold(gray, 128, 255, cv2.THRESH_BINARY)
#     print("DEBUG: Applied binary thresholding with threshold=80.")
...
#     # Find ID contour based on configurable area range - area-based geometric selection
#     id_contour = None
#     if min_id_area is not None and max_id_area is not None:
#         for i, contour in enumerate(sorted_contours):
#             area = cv2.contourArea(contour)
...
#     # Find OD contour based on configurable area range - area-based geometric selection
#     od_contour = None
#     if min_od_area is not None and max_od_area is not None:
#         for i, contour in enumerate(sorted_contours):
#             area = cv2.contourArea(contour)
...
#         if contour is None or len(contour) < 5:
#             print("DEBUG: Contour is None or has insufficient points (<5).")
#             return None
...
#     if id_contour is None:
#         print(f"DEBUG: ID contour not provided, searching in contours based on area range ({min_id_area}-{max_id_area}).")
#         for i, contour in enumerate(sorted_contours):
#             area = cv2.contourArea(contour)
#             if min_id_area <= area <= max_id_area:
#                 id_contour = contour
...
#     if od_contour is None:
#         print(f"DEBUG: OD contour not provided, searching in contours based on area range ({min_od_area}-{max_od_area}).")
#         for i, contour in enumerate(sorted_contours):
#             area = cv2.contourArea(contour)
#             if min_od_area <= area <= max_od_area:
#                 od_contour = contour

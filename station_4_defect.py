...
# NEW: Adds processing-contour gating so all steps run inside a contour with area in [600000, 700000].
...
# Processing-contour gating range (pixels^2)
PROCESS_MIN_AREA = 600000
PROCESS_MAX_AREA = 700000
...
def preprocess_image(frame, min_id_area=None, max_id_area=None,
                     ...
                     output_folder=None):
    """Preprocessing with processing-contour gating, area + shape filters to select ID/OD contours."""
    if output_folder:
    ...
    # First pass threshold (global fixed) to locate processing contour
    _, binary = cv2.threshold(gray, 80, 255, cv2.THRESH_BINARY)
    binary_inv = cv2.bitwise_not(binary)

    contours_all, _ = cv2.findContours(binary_inv, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    sorted_all = sorted(contours_all, key=cv2.contourArea, reverse=True)

    # Find processing contour by area in [PROCESS_MIN_AREA, PROCESS_MAX_AREA]
    processing_contour = None
    for c in sorted_all:
        a = cv2.contourArea(c)
        if PROCESS_MIN_AREA <= a <= PROCESS_MAX_AREA:
            processing_contour = c
            break

    # If not found, return minimal info; main() will handle error and fallbacks
    if processing_contour is None:
        contour_img = frame.copy()
        return {
            "image": frame.copy(),
            "sorted_contours": [],  # No masked contours
            "original_gray": gray,
            "id_contour": None,
            "od_contour": None,
    ...
    # Build processing mask and re-threshold inside it for stable downstream ops
    proc_mask = _filled_mask_from_contour(gray, processing_contour)
    masked_gray = cv2.bitwise_and(gray, gray, mask=proc_mask)
    _, masked_binary = cv2.threshold(masked_gray, 80, 255, cv2.THRESH_BINARY)
    masked_binary_inv = cv2.bitwise_not(masked_binary)
    contours, _ = cv2.findContours(masked_binary_inv, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    sorted_contours = sorted(contours, key=cv2.contourArea, reverse=True)

    # ID/OD selection under area + shape filters
    id_contour = None
    ...
    contour_img = frame.copy()
    # Draw processing contour boundary in yellow to visualize gating region
    cv2.drawContours(contour_img, [processing_contour], -1, (0, 255, 255), 3)
    ...
    masked_image = cv2.bitwise_and(frame, frame, mask=proc_mask)

    return {
        "image": masked_image.copy(),          # Use masked image for downstream fallbacks/visuals
        "sorted_contours": sorted_contours,    # Contours detected within processing mask
        "original_gray": gray,
        "id_contour": id_contour,
        "od_contour": od_contour,
        "contour_image": contour_img,
        "processing_contour": processing_contour,
        "processing_mask": proc_mask,
...
def detect_burr(frame, sorted_contours, ID2_OFFSET=None, HIGHLIGHT_SIZE=None,
    ...
    """Legacy ID-only detection kept for compatibility; expects frame already masked by processing contour."""
...
def detect_burr_both(frame, sorted_contours,
                     ...
                     min_aspect_ratio=None, max_aspect_ratio=None,
                     output_folder="output_images", id_contour=None, od_contour=None):
    """ID+OD burr detection with combined overlays and union crop; expects frame already masked by processing contour."""
    ...
    if od_contour is not None:
        ...
        # Correct keyword name burr_area_max

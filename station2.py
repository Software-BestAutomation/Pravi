# code 2
# Changes:
# - Added backup_output_folder parameter to main for configurable timestamped backups.
# - Added _write_backup() helper and invoked it wherever we save or fallback-save images.
# - save_thickness_result_image is called with backup_output_folder to write a timestamped copy in addition to cam2_bmp.bmp.
# - Legacy primary save (cam2_bmp.bmp in output_folder) is preserved.
...
def _write_backup(frame, backup_output_folder):
    """Write a timestamped backup copy if a backup folder is provided."""
    try:
        if backup_output_folder and frame is not None:
            ...
            ok = cv2.imwrite(backup_path, frame)
            print(f"Backup wrote to {backup_path}: {ok}")
    except Exception as _be:
        print(f"Backup write error: {_be}")

def main(part, subpart, frame, thick_min, thick_max, pixel_to_micron, output_folder, min_thresh, max_thresh,
    ...
    if part in excluded_parts:
        # For excluded parts, return NA without processing, but ensure legacy save + backup
        try:
            ...
            if frame is not None:
                ...
                _write_backup(frame, backup_output_folder)  # NEW
        ...
        try:
            ...
            # Final guard: ensure cam2_bmp.bmp exists and write backup
            try:
                os.makedirs(output_folder, exist_ok=True)
            ...
            # On any processing error, attempt fallback save + backup
            try:
                os.makedirs(output_folder, exist_ok=True)
                if frame is not None:
                    fallback_path = os.path.join(output_folder, "cam2_bmp.bmp")
